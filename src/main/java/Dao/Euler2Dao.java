...
public class Euler2Dao implements Esqueleto {
    ...
    @Override
    public double[][] ejecucion_matriz(double h, String funcionA, String funcionB, double inicialx, double inicialy, double N) {
        double[][] array = new double[(int) N][2];
        array[0][0] = inicialx;
        array[0][1] = inicialy;
        for (int i = 1; i < N; i++) {
            array[i][0] = array[i - 1][0] + (h * funcion(funcionA, array[i - 1][0], array[i - 1][1]));
            array[i][1] = array[i - 1][1] + (h * funcion(funcionB, array[i - 1][0], array[i - 1][1]));
        }
        return array;
    }

    @Override
    public void salida(double[] t, double[][] xy, int N) {
        DecimalFormat formato = new DecimalFormat("##00.000000");
        DecimalFormat form = new DecimalFormat("#0.00");
        System.out.println("iteraccion" + "\t" + "\t" + "T" + "\t" + "\t" + "\t" + "X" + "\t" + "\t" + "\t" + "Y");
        for (int i = 0; i < N; i++) {
    ...
    @Override
    public void grafica(String name, double[][] xy, double[] t, String funcionA, String funcionB) {
        ...
        grafica.addLinePlot(funcionA, t, x);
        grafica.addLinePlot(funcionB, t, y);
    ...
    @Override
    public double funcion(String funcion, double x, double y) {
        JEP jep = new JEP();
